...
const i = t.split('\n')
i.shift()
for (let e = 0; e < i.length; ++e) i[e] = i[e].split(',')
let a = [],
  o = [],
  l = [],
  n = 0
    ...
    var a = i[e],
      o =
        ...
        s.getShaderParameter(o, s.COMPILE_STATUS)
          ? o
          : (alert("Didn't compile shader. Info: " + s.getShaderInfoLog(o)),
            s.deleteShader(o),
            null))
    t.attachShader(r, o)
  }
  return (
    null != e && t.transformFeedbackVaryings(r, e, t.INTERLEAVED_ATTRIBS),
    t.linkProgram(r),
    t.getProgramParameter(r, t.LINK_STATUS)
      ? r
      : (alert("Didn't link shader. Info: " + t.getProgramInfoLog(r)),
...
class p {
  ...
  constructor(e, t) {
    ...
    for (let e = 0; e < this.vao_desc.length; e++)
      !(function (i, s, e, r) {
        ...
        i.bindVertexArray(null), i.bindBuffer(i.ARRAY_BUFFER, null)
  ...
  updateParticlesHelper() {
    this.frame++,
      ...
      this.gl.uniform1fv(
        this.gl.getUniformLocation(this.updateParticles, 'v'),
        this.lerpParams
      ),
    ...
    this.gl.uniform1fv(
      this.gl.getUniformLocation(this.updateParticles, 'mps'),
      this.mousePositions
    ),
      this.gl.uniform1i(
        this.gl.getUniformLocation(this.updateParticles, 'frame'),
        this.frame
      ),
      this.gl.uniform1i(
        this.gl.getUniformLocation(this.updateParticles, 'pen'),
        0 | (d || r)
      ),
      ...
      this.gl.uniform2f(
        this.gu(this.updateParticles, 'i_dim'),
        this.params.simSize,
        this.params.simSize
      ),
  ...
  blurHelper() {
    this.gl.disable(this.gl.BLEND), this.gl.bindVertexArray(this.vao)
    var t = Math.round(this.lerpParams[16])
    for (let e = 0; e < t; e++)
      this.gl.useProgram(this.updateBlur),
        this.gl.uniform1fv(
          this.gl.getUniformLocation(this.updateBlur, 'v'),
          this.lerpParams
        ),
  ...
  drawParticlesToCanvas() {
    this.gl.bindVertexArray(this.vaos[this.read + 2]),
      ...
      this.gl.uniform1f(
        this.gu(this.renderParticles, 'pointsize'),
        this.params.drawPointsize
      ),
      this.gl.uniform1f(
        this.gu(this.renderParticles, 'dotSize'),
        this.lerpParams[19]
  ...
  fadeScreen() {
    this.gl.useProgram(this.clearScreen),
      this.gl.enable(this.gl.BLEND),
      this.gl.uniform1fv(
        this.gl.getUniformLocation(this.clearScreen, 'v'),
        this.lerpParams
      ),
      this.gl.bindTexture(this.gl.TEXTURE_2D, this.screenTexture),
      this.gl.bindFramebuffer(this.gl.FRAMEBUFFER, this.framebuffer),
  ...
  drawCanvasToScreen() {
    this.gl.useProgram(this.drawScreen),
      this.gl.uniform1i(
        this.gu(this.drawScreen, 'invert'),
        27 == s || (36 == s) | 0
      ),
      ...
      h
        ? this.gl.viewport(
            0,
            parseInt(this.params.renderSize),
            this.params.renderSize,
            this.params.renderSize
          )
        : this.gl.viewport(
  ...
  draw() {
    ;(this.params.lerpTime = Math.min(
      1,
      this.params.lerpTime +
        Math.max(
          (1 - this.params.lerpTime) * this.params.convergenceRate,
          0.001
        )
    )),
      ...
      this.params.displayParticles
        ...
        : (this.gl.useProgram(this.drawBlur),
          ...
          this.gl.uniform2f(
            this.gu(this.drawBlur, 'uTextureSize'),
            this.params.simSize,
            this.params.simSize
          ),
          this.gl.clearColor(0, 0, 0, 1),
  ...
  _initVertexArray() {
    ...
    this.gl.bindBuffer(this.gl.ARRAY_BUFFER, t),
      ...
      this.gl.enableVertexAttribArray(
        this.ga(this.drawBlur, 'aVertexPosition')
      ),
      ...
      this.gl.enableVertexAttribArray(
        this.ga(this.updateBlur, 'aVertexPosition')
      ),
      this.gl.vertexAttribPointer(
        this.ga(this.updateBlur, 'aVertexPosition'),
        2,
    ...
    return (
      ...
      this.gl.vertexAttribPointer(
        this.ga(this.drawBlur, 'aTexCoord'),
        2,
        this.gl.FLOAT,
        !1,
        0,
      ...
      this.gl.vertexAttribPointer(
        this.ga(this.updateBlur, 'aTexCoord'),
        2,
        this.gl.FLOAT,
        !1,
        0,
...
function R() {
  ...
  !(function (e) {
    ...
    new Date()
    ;(t.download = e + '.png'), (t.href = T.toDataURL()), t.click(), t.remove()
  })('36_Points_' + f)
}
const A = {
  simSize: 512,
...
h && ((A.drawPointsize = 512 / 330), (A.simSize = 330))
...
function drop(ev) {
  ...
  if (ei === '🥚' && arr.includes(e - 1) && e - 1 == s) d = true
}
...
function P() {
  A.mobile
    ...
    : (A.canvasZoom = Math.max(
        window.innerHeight / A.renderSize,
        (0.5 * window.innerWidth) / A.renderSize
      )),
...
b || alert('Requires WebGL2'), P()
let F = [
  49, 50, 51, 52, 53, 54, 55, 56, 57, 48, 113, 119, 101, 114, 116, 121, 117,
  105, 111, 112, 97, 115, 100, 102, 103, 104, 106, 107, 108, 122, 120, 99, 118,
  98, 110, 109, 90, 88, 67, 86, 66, 78, 77
]
function S() {
...
function w(e) {
  e < 0 || e >= o.length
    ...
    : ((s = e),
      ...
      _(),
      console.log(
        'Point ' + (e + 1).toString().padStart(2, '0') + ' (' + l[e] + ')'
      ))
}
...
function D(e) {
  ...
  '\\' == t
    ? ((A.params = U.lerpParams), (A.lerpTime = 1), console.log('Frozen!'))
    : 'U' == t
    ? (A.update = !A.update)
    : 'S' == t
    ? (R(), console.log('Save canvas'))
    : 'R' == t
    ? ((U = new p(b, A)), console.log('Reset'))
    : 'L' == t
    ? console.log('Current parameters: ' + this.lerpParams)
    : ' ' == t
    ? (E(), console.log('Toggle fullscreen'))
    : '~' == t
    ? y(
    ...
    ? ((A.convergenceRate = Math.min(1, 2 * A.convergenceRate)),
      console.log('Convergence speed: ' + A.convergenceRate))
    : (1e3 < n &&
        (109 === e && 0.95 < Math.random()
          ? (e = 86)
          : 104 === e && 0.95 < Math.random()
          ? (e = 67)
          : 110 === e && 0.99 < Math.random()
          ? (e = 90)
          : 97 === e && 20 == s && (e = 78)),
      w(F.indexOf(e)))
}
window.addEventListener('resize', P),
  (document.onkeypress = function (e) {
    ;(B = 0), D((e = e || window.event).keyCode || e.which)
  })
let U = new p(b, A)
let B = 0
...
function C() {
  B++,
    ...
    18e3 < B &&
      s != e - 1 &&
      (console.log(
        'Slow mode (~5 minutes of inactivity)... returning to original edition.'
      ),
